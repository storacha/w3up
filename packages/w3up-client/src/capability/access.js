 ...
 *
 * @param {API.Agent<API.AccessService>} agent - Agent connected to the w3up service.
 * @param {object} input
...
export const delegate = async (
  agent,
  { delegations, proofs = [], space = agent.data.currentSpace }
) => {
  if (!space) {
...
class PendingAccessRequest {
   ...
   * @property {API.Agent<API.AccessService>} agent - Agent handling interaction.
...
export class GrantedAccess {
   ...
   * @property {API.Agent<API.AccessService>} agent - Agent that processed the request.
