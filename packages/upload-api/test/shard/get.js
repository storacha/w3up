 ...
 */
export const test = {
  'shard/get returns information about an uploaded CID': async (assert, context) => {
    ...
    const storeAdd = await Store.add.invoke({
      issuer: alice,
      audience: context.id,
      with: spaceDid,
      nb: { link, size },
    ...
    assert.ok(shardGet.out.ok, `failed to get shard: ${shardGet.out.error?.message}`)
