...
export const test = {
  ...
  'can requestAuthorization': async (assert, context) => {
    const { agent, mail, account, email } = await setup(context)
  ...
  'authorize and wait': async (assert, context) => {
    const { email, account, agent, service, mail } = await setup({
      ...context,
      email: 'example+123@dag.house',
    })

    const deviceA = agent
    const authorized = authorizeAndWait(deviceA, email, {
    ...
    assert.deepEqual(confirmationEmail.to, email, 'emails are equal')
  },
  'authorize with poll claim': async (assert, context) => {
    ...
    const authorized = authorizeAndWait(agent, email, {
      expectAuthorization: waitForAuthorizationByPolling,
    })
    const confirmationEmail = await mail.take()
    await confirmConfirmationUrl(agent.connection, confirmationEmail)
...
/**
 * @param {API.TestContext & { email?:`${string}@${string}` }} context
 */
const setup = async ({ email = 'alice@web.mail', ...context }) => {
  const space = alice
  const account = Absentee.from({ id: DidMailto.fromEmail(email) })
  ...
  return { email, space, account, agent, ...context }
}
...
/**
 * @param {API.ConnectionView<API.AccessAPI.Service>} connection
