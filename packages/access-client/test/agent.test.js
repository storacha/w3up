...
describe('Agent', function () {
  ...
  it('should revoke', async function () {
    ...
    assert(result2.ok, `failed to revoke when proofs passed: ${result2.error?.message}`)
    ...
    assert(result3.error, `revoke resolved but should have rejected because delegation is not passed`)

    // 
    const result4 = await alice.revoke(bobDelegation.cid, { proofs: [bobDelegation] })
    assert(result4.ok, `failed to revoke even though proof was passed: ${result4.error?.message}`)
  })
})
